...
SECRETS_FILTER = SCRIPT_DIR / "secrets-filter"
STANDALONE_OUTPUT = SCRIPT_DIR / "secrets-filter-standalone"
...
def extract_filter_logic() -> str:
    """Extract filter logic from secrets-filter, removing import/auto-gen block.

    Reads the secrets-filter script and extracts everything after the
    ...
    if not SECRETS_FILTER.exists():
        raise FileNotFoundError(f"secrets-filter not found: {SECRETS_FILTER}")
    ...
    if start_idx is None:
        raise ValueError("Could not find VALID_FILTERS in secrets-filter")
...
def generate_standalone() -> str:
    """Generate standalone secrets-filter with patterns baked in."""
    source_hash = compute_source_hash()
    timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
