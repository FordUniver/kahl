...
Dependencies: Python stdlib only (re, os, sys, math, collections).
...
Entropy filter options:
  --entropy-threshold=N   - override all thresholds (default varies by charset)
  --entropy-min-length=N  - minimum token length (default: 16)
  --entropy-max-length=N  - maximum token length (default: 256)
...
_entropy_yaml = SCRIPT_DIR.parent / "patterns" / "entropy.yaml"
...
def _should_regenerate() -> bool:
    ...
    for src in [_patterns_yaml, _env_yaml, _entropy_yaml]:
