...
# Use raw bytes for I/O to handle binary data correctly
# UTF-8 encoding happens at the text processing layer, not I/O layer
binmode(STDIN,  ':raw');
binmode(STDOUT, ':raw');
...
my $STATE_IN_PRIVATE_KEY_OVERFLOW = 2;
