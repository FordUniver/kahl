...
echo "Building kahl v$VERSION for $PLATFORM (mode: $BUILD_MODE)"
echo ""
...
if [[ "$BUILD_MODE" == "standalone" || "$BUILD_MODE" == "all" ]]; then
  ...
  # --- Compiled languages (platform-specific naming) ---
  ...
  if should_build "rust"; then
    ...
    cp "$REPO_ROOT/rust/target/release/kahl" "$OUT_DIR/kahl-rust-$PLATFORM-$VERSION"
  fi
  ...
  if should_build "go"; then
    ...
    mkdir -p "$OUT_DIR"
    (cd "$REPO_ROOT/go" && go build -ldflags="-s -w -X main.version=$VERSION" -o "$OUT_DIR/kahl-go-$PLATFORM-$VERSION" main.go patterns_gen.go)
  fi
  ...
  if should_build "swift"; then
    ...
    (cd "$REPO_ROOT/swift" && swiftc -O -whole-module-optimization -o "$OUT_DIR/kahl-swift-$PLATFORM-$VERSION" main.swift patterns_gen.swift version_gen.swift)
  fi

  # --- Script languages (platform-independent naming) ---
